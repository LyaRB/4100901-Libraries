...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart){ //librería para la interruppción de los bytes que se reciben
	if(huart -> Instance == USART2){ //USART2 es un apuntador a una dirección, Instance se refiere a la dirección del periferico para USART2
...
int main(void)
{
  ...
  HAL_UART_Receive_IT(&huart2, &ID, 1);
  while (1)
  {
    /* USER CODE END WHILE */
	  if(ring_buffer_size() != 0){
	  		  ...
	  		  if(ring_buffer_is_full()){ //si está lleno se comienzan a leer los datos
	  			  ...
	  			  for (uint8_t i = 0; i <= 11; i++){//recorre cada espacio para leerlo
	  				  ring_buffer_read(&byte); //cambia el limite de i
